...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    @Override//BCryptPasswordEncoder的联合写法，从数据库拿用户名
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(hrService)
                .passwordEncoder(new BCryptPasswordEncoder());
    }

    @Override//配置拦截资源，例如过滤掉css/js/images等静态资源
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/index.html", "/static/**", "/login_p", "/favicon.ico", "/swagger-ui.html", "/webjars/**", "/swagger-resources/**", "/v2/**", "/passwordForget/**", "/register/**");
    }
...
//                loginProcessingUrl("/login")处理请求的接口。
